...
//     if (searchText) {
//       router.push(`/search?query=${searchText}`, undefined, { scroll: false });
//       handleLogEvent("search", `searched ${searchText}`);
//     } else {
//       router.push(`/`, undefined, { scroll: false }); // Optional: If search text is empty, redirect to homepage
//     }
...
//     <>
//       <CartDrawer />
//       <div className="sticky top-0 z-20" style={{ backgroundColor: "#1A4B8B" }}>
...
const Navbar = () => {
  ...
  const handleSubmit = (e) => {
    ...
    searchInputRef.current.focus(); // Focus back to the search input
    setSearchText(""); // Optional: Clear the search text after submit
  };
              ...
              <div className="w-full flex flex-col justify-center flex-shrink-0 relative z-30">
                <div className="flex flex-col mx-auto w-full">
                  ...
                  >
                    <label className="flex items-center py-0.5">
                      <input
                        ref={searchInputRef} // Add the reference to the input field
                        onChange={(e) => setSearchText(e.target.value)}
                        value={searchText}
                        className="form-input w-full pl-5 appearance-none transition ease-in-out border text-input text-sm font-sans rounded-md min-h-10 h-10 duration-200 bg-white focus:ring-0 outline-none border-none focus:outline-none placeholder-gray-500 placeholder-opacity-75"
                        placeholder="Search for products...."
