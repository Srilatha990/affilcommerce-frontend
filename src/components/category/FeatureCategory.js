      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-4 p-4">
          {data[0]?.children?.map((category, i) => (
            <div className="flex justify-center items-center" key={i}>
              <div className="flex flex-col items-center justify-center">
                {/* Category Image with hover effect and onClick functionality */}
                <div
                  className="w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 mb-3 bg-white border-4 border-gray-100 rounded-full flex justify-center items-center shadow-lg group hover:scale-110 hover:shadow-2xl transition-all duration-300 ease-in-out cursor-pointer"
                  onClick={() =>
                    handleCategoryClick(
                      category._id,
                      showingTranslateValue(category?.name)
